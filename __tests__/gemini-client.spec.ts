...
describe("Gemini Fetch Client (integration, optional)", () => {
  ...
  maybe(
    "generateContent returns a candidate and maps to OpenAI compat",
    async () => {
      const client = new GeminiFetchClient({ apiKey: API_KEY! });
      const res = await client.listModels();
      const model =
        res.models?.[0]?.name?.replace(/^models\//, "") || "gemini-1.5-flash";
      const gen = await client.generateContent(model, {
