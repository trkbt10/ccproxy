...
import { grokToOpenAIResponse } from "../src/adapters/providers/grok/openai-response-adapter";
import { grokToOpenAIStream } from "../src/adapters/providers/grok/openai-response-adapter";
describe("Converters: Gemini -> OpenAI", () => {
  it("geminiToOpenAIResponse maps text and usage", () => {
    ...
    expect(out.output?.[0] && (out.output[0] as any).content?.[0]?.text).toBe(
      "Answer"
    );
  ...
  it("geminiToOpenAIStream produces delta events in order", async () => {
    async function* fakeGemini() {
      ...
      yield {
        candidates: [{ content: { parts: [{ text: "Hello Wor" }] } }],
      } as any;
      yield {
